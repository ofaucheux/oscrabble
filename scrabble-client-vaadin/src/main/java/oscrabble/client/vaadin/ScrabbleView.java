...
public class ScrabbleView extends VerticalLayout implements BeforeEnterObserver {
	...
	private final static ImageGenerator IMAGE_FACTORY = new ImageGenerator();
	...
	private Context ctx;
	private DictionaryDialog dictionaryEntriesDialog;
	...
	private void setModus(Modus modus) {
		...
		switch (modus) {
			case DICTIONARY:
				this.grid.getElement().getStyle().set("cursor", "help");
				break;
			default:
				this.grid.getElement().getStyle().remove("cursor");
		}
	}
			...
			if (square.tile != null) {
				...
				html.append(
						getHtmlPositionedImgCode(
								...
								1,
								1,
								IMAGE_FACTORY.generateTileImage(square.tile, flash)
						));
			}
		}
		...
		if (preparedAction != null) {
			...
			for (final Triple<Integer, Integer, Tile> t : preparedTiles) {
				...
				html.append(
						getHtmlPositionedImgCode(
								...
								1,
								1,
								IMAGE_FACTORY.generateTileImage(t.getRight(), false)
						)
				);
			}
			...
			html.append(
					getHtmlPositionedImgCode(
							...
							1,
							1,
							IMAGE_FACTORY.generateDirectionArrowImage(preparedAction.getDirection())
					)
			);
		}
		...
		for (int i = 0; i < rack.tiles.size(); i++) {
			...
			html.append(
					createHtmlEmbeddedImgCode(
							CELL_DIMENSION,
							IMAGE_FACTORY.generateTileImage(rack.tiles.get(i), false),
							""
					)
			);
		}
	...
	class GridComponent extends Div {
		...
		private void handleClick(DomEvent event) {
			...
			if (ScrabbleView.this.modus == Modus.DICTIONARY) {
				ScrabbleView.this.dictionaryEntriesDialog.showEntries(column, row);
				setModus(Modus.NORMAL);
				return;
			}
	...
	private class DictionaryDialog extends Dialog {
		...
		private void showEntries(char column, int row) {
			final IDictionary dictionary = ScrabbleView.this.ctx.game.getDictionary();
			this.layout.removeAll();
			final Set<String> words = ScrabbleView.this.ctx.game.getGrid().getWords(String.format("%s%s", column, row));
			if (!words.isEmpty()) {
				open();
			}
			for (final String word : words) {
				this.layout.add(new H2(word));
				try {
					final DictionaryEntry entry = dictionary.getEntry(word);
					entry.getDefinitions().stream()
							.map(Label::new)
							.forEach(this.layout::add);
				} catch (DictionaryException e) {
					this.layout.add(new Label(e.getLocalizedMessage()));
				}
			}
		}
	}
